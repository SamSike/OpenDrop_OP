...
from modules.classes import ExperimentalSetup, CAExperimentalDrop, IFTExperimentalDrop
...
class FunctionWindow(CTk):
    def __init__(self, function_type, fitted_drop_data):
        ...
        self.ca_experimental_drop = CAExperimentalDrop()
        self.ift_experimental_drop = IFTExperimentalDrop()
    ...
    def widgets(self, function_type, user_input_data, fitted_drop_data):
        ...
        if function_type == FunctionType.PENDANT_DROP:
            user_input_data.function = FunctionType.PENDANT_DROP
            self.ift_acquisition_frame.pack(fill="both", expand=True)
        elif function_type == FunctionType.CONTACT_ANGLE:
            user_input_data.function = FunctionType.CONTACT_ANGLE
    ...
    def next(self, function_type, user_input_data, fitted_drop_data):
        ...
        if self.current_stage == Stage.PREPARATION:
            ...
            if function_type == FunctionType.PENDANT_DROP:
                ...
                self, user_input_data, self.ift_experimental_drop, fg_color=self.FG_COLOR)
                self.ift_preparation_frame.pack(fill="both", expand=True)
            else:
                ...
                self, user_input_data, self.ca_experimental_drop, fg_color=self.FG_COLOR)
                self.ca_preparation_frame.pack(fill="both", expand=True) 
            ...
            if function_type == FunctionType.PENDANT_DROP:
                ...
                validation_messages = validate_user_input_data_cm(user_input_data,self.ca_experimental_drop)
