...
class CaAnalysis(CTkFrame):
        ...
        try:
            ...
            if method_enum in [FittingMethod.CIRCLE_FIT, FittingMethod.ELLIPSE_FIT]:
                ...
                if BASELINE_INTERCEPTS in angles_data:
                    ...
                    self.create_contact_points_and_tangents(angles_data, baseline_intercepts, left_angle, right_angle)
                    
            elif method_enum == FittingMethod.POLYNOMIAL_FIT:
                # Special handling for polynomial fit
                if CONTACT_POINTS in angles_data:
                    self.create_tangent_lines_from_angles(angles_data, left_angle, right_angle)
    ...
    def initialize_image_display(self, frame):
        ...
        self.contact_method_label = CTkLabel(self.contact_method_frame, text="Contact Angle Method:")
        self.contact_method_label.grid(row=0, column=0, padx=(10, 5))
        
        self.selected_contact_method = StringVar()
        self.contact_method_dropdown = CTkComboBox(self.contact_method_frame, 
        ...
        self.chart_method_label = CTkLabel(self.chart_method_frame, text="Chart Method:")
        self.chart_method_label.grid(row=0, column=0, padx=(10, 5))
        
        self.selected_chart_method = StringVar()
        self.chart_method_dropdown = CTkComboBox(self.chart_method_frame, 
    ...
    def is_ml_fit_method(self, method):
        ...
        method_str = method if isinstance(method, str) else (method.value if hasattr(method, 'value') else str(method))
        ml_fit_names = ['ml_fit', 'ML_FIT', 'ml fit', 'ML Fit', 'machine_learning', 'ML']
    ...
    def update_contact_method_dropdown(self):
        ...
        if self.current_index in self.available_methods:
            ...
            if methods:
                ...
                if not self.selected_contact_method.get() or self.selected_contact_method.get() not in methods:
                    self.contact_method_dropdown.set(methods[0])
                else:
                    self.contact_method_dropdown.set(self.selected_contact_method.get())
            else:
                self.contact_method_dropdown.configure(values=["No visualization methods available"])
    ...
    def update_chart_method_dropdown(self):
        ...
        if self.current_index in self.preformed_methods:
            ...
            # Set default selection
            if all_methods:
                if not self.selected_chart_method.get() or self.selected_chart_method.get() not in all_methods:
        ...
        if self.current_index in self.available_chart_methods:
            ...
            if methods:
                ...
                # Set default selection
                if not self.selected_chart_method.get() or self.selected_chart_method.get() not in methods:
    ...
    def display_line_chart(self):
        ...
        if selected_method in self.method_angles:
            ...
            if not valid_indices:
                self.image_label.configure(text=f"No valid angle data for method: {selected_method}", image="")
                return
        else:
