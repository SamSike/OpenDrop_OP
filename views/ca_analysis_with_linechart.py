...
class CaAnalysis(CTkFrame):
                ...
                if method_to_use:
                    ...
                    if left_angle is not None and right_angle is not None:
                        ...
                        if contact_points is not None and tangent_lines is not None and index in self.cropped_images:
                            ...
                            #append left and right angles to the list
    ...
    def initialize_image_display(self, frame):
        ...
        # Use Radiobuttons for mutually exclusive options
        self.view_option_var = IntVar(value=0)  # 0 for original, 1 for cropped, 2 for line chart

        # Add Radiobutton for original image as default option
        self.show_original_rb = CTkRadioButton(
            self.toggle_frame, 
            text="Show Original Image", 
            variable=self.view_option_var, 
            value=0, 
            command=self.toggle_view
        )
        self.show_original_rb.grid(row=0, column=0, padx=10, pady=5)

        # Adjust positions of other Radiobuttons
        self.show_angles_rb = CTkRadioButton(
            self.toggle_frame, 
        ...
        self.show_angles_rb.grid(row=0, column=1, padx=10, pady=5)

        self.show_line_chart_rb = CTkRadioButton(
            self.toggle_frame, 
            text="Show Line Chart (Blank View)", 
            variable=self.view_option_var, 
            value=2, 
            command=self.toggle_view
        )
        self.show_line_chart_rb.grid(row=0, column=2, padx=10, pady=5)
    ...
    def toggle_view(self):
        """Toggle between original image, cropped image with angles, and line chart"""
        if self.view_option_var.get() == 1:
            self.display_cropped_image()
        elif self.view_option_var.get() == 2:
    ...
    def display_line_chart(self):
        """Display a line chart of contact angles over frames"""
        # Prepare data for plotting
        
        print(f"Left Angles1: {self.left_angles}")
        print(f"Right Angles1: {self.right_angles}")
        frames = list(range(1, len(self.left_angles) + 1))

        # Create a figure and axis
        fig, ax = plt.subplots(figsize=(5, 4))  # 调整figsize以增加图像大小
        fig.tight_layout()  # 自动调整布局
        ax.plot(frames, self.left_angles, color='orange', label='Left Angle', marker='o')
        ax.plot(frames, self.right_angles, color='blue', label='Right Angle', marker='o')
        ax.set_xlabel('Frame')
        ax.set_ylabel('Angle (degrees)')
        ax.set_title('Contact Angles Over Frames')
        ax.set_xticks(frames)  # Set x-ticks to match frame numbers
        ax.legend()
        ...
        image = Image.frombytes('RGB', (width, height), canvas.tostring_rgb())

        # Display the image
        self.tk_image = CTkImage(image, size=(400, 300))
        self.image_label.configure(image=self.tk_image)
        self.image_label.image = self.tk_image
        plt.close(fig)
