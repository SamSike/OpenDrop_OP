...
RotatedRect = Tuple[Vector2[float],
                    Vector2[float], Vector2[float], Vector2[float]]
...
def extract_pendant_features(
    ...
    if drop_region is not None:
        drop_image = image[drop_region.y0:drop_region.y1 +
                           1, drop_region.x0:drop_region.x1+1]
        print(
    ...
    if needle_region is not None:
        needle_image = image[needle_region.y0:needle_region.y1 +
                             1, needle_region.x0:needle_region.x1+1]
    ...
    if needle_image is not None:
        ...
        if needle_fit_result is not None:
            ...
            if n_lpoints > 0 and n_rpoints > 0 and abs(n_lpoints - n_rpoints)/(n_lpoints + n_rpoints) < 0.33:
                ...
                needle_min_z = min(needle_lpoints_z.min(),
                                   needle_rpoints_z.min())
                needle_max_z = max(needle_lpoints_z.max(),
                                   needle_rpoints_z.max())
    ...
    return drop_points, needle_diameter_px, drop_region, needle_region, image, drop_image, needle_fit_result
...
def _largest_connected_component(gray: np.ndarray) -> np.ndarray:
    ...
    ix = np.argsort(stats[:, cv2.CC_STAT_WIDTH] *
                    stats[:, cv2.CC_STAT_HEIGHT])[::-1]
    if len(ix) > 1:
        if ix[0] == 0:
...
def find_pendant_apex(data: Tuple[np.ndarray, np.ndarray]) -> Optional[tuple]:
    ...
    asymm_r = (np.convolve((bowl_z - bowl_z.mean())
               [bowl_r_ix], ma_kernel, mode='valid')**2).sum()
    asymm_z = (np.convolve((bowl_r - bowl_r.mean())
               [bowl_z_ix], ma_kernel, mode='valid')**2).sum()
    if asymm_z > asymm_r:
