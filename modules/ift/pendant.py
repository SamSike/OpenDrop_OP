...
def extract_pendant_features(
    ...
    if image is None:
        print(f"Could not load image at {image}")
    drop_region, needle_region = get_ift_regions(image)

    if drop_region is not None:
        drop_image = image[drop_region.y0:drop_region.y1+1, drop_region.x0:drop_region.x1+1]
        print(f"Drop region: {drop_region.x0}, {drop_region.y0}, {drop_region.x1}, {drop_region.y1}")
...
def analyze_ift(
    ...
    *,
    ...
    worthington = ((delta_density * gravity * volume) / (PI * IFT * needle_diameter_mm))/1000
