...
def auto_crop(img: np.ndarray, low=50, high=150, apertureSize=3, verbose=0):  # DS 08/06/23
    '''
    ...
    if lines is not None:  # if the HoughLines function is successful
        ...
        if discriminant > 0:
            ...
            intersect1, intersect2 = (
                dx * t1 + p1[0], dy * t1 + p1[1]), (dx * t2 + p1[0], dy * t2 + p1[1])
...
def prepare_hydrophobic_new(coords, xi=0.8, display=False):
    ...
    for halfdrop in [l_drop, r_drop]:
        ...
        div_line_value = min(
            halfdrop[:, [1]]) + (max(halfdrop[:, [1]]) - min(halfdrop[:, [1]]))*percent
        for n in halfdrop:
        ...
        if 1:
            ...
            xnew_halfdrop = new_halfdrop[:, [0]].reshape(
                len(new_halfdrop[:, [0]]))
            ynew_halfdrop = new_halfdrop[:, [1]].reshape(
                len(new_halfdrop[:, [1]]))
...
def prepare_hydrophobic(coords, xi=0.8, display=False):
    ...
    if display:
        ...
        plt.title(str(len(dic.keys())) +
                  ' groups found by clustering with epsilon value of '+str(eps))
        plt.show()
        plt.close()
    maxkey = max(dic, key=lambda k: len(dic[k]))
    longest = dic[maxkey]
...
def extract_edges_CV(img: np.ndarray) -> np.array:
    '''
...
def tilt_correction(img: np.ndarray, baseline):
...
def preprocess(img: np.ndarray, display=False):
...
def save_to_new_dpi(img: np.ndarray, dpi=256):
    plt.imshow(img)
    plt.axis('off')
...
def prepare4model_v03_img(img: np.ndarray, cluster=True, input_len=1223, right_only=False, display=False):
    ...
    if cluster == True:
        ...
        eps = 3
        dic, dic2 = cluster_optics(edges, eps=eps), cluster_optics(
            edges, out_style='xy', eps=eps)
    ...
    for coords in [flipped, coords.copy()]:  # for flipped and right side
        ...
        if len(X) > max_contour_len:  # was input_len but was lowered to the max scaler 2 contour len
            if display == True:
                ...
                target_dpi = 100
                print("Contour of length "+str(len(X)) +
            ...
            print(
                'WARNING: contour shorter than shortest training data, inaccurate predictions likely')
    ...
    if repeat == True:  # repeat the above with the resolution dropped
        ...
        for coords in [flipped, coords.copy()]:  # for flipped and right side
            ...
            if len(X) > max_contour_len:
                ...
                print("Half-drop contour of length "+str(len(X)) +
                      " is too long for the designated output dimensionality of ("+str(input_len)+",2)")
                ...
                print(
                    'WARNING: half-drop contour shorter than shortest training data, inaccurate predictions likely')
                print('length of input is '+str(len(X)))
            else:
                print('Half-drop contour of length ' +
                      str(len(X))+' is now in range')
...
def prepare4model_v03(coords, input_len=1223, right_only=False, display=False):
    ...
    for coords in [flipped, coords.copy()]:  # for flipped and right side
        ...
        top = []  # will need this later
        div_line_value = min(
            coords[:, 1]) + (max(coords[:, 1]) - min(coords[:, 1]))*percent
        for n in coords:
            if n[1] < div_line_value:
                bottom.append(n)
            else:
...
def experimental_pred(pred_ds, model, side='both', cluster=True, display=False):
    ...
    if side == 'left':
        # Convert to float32
        input_left = np.array([pred_ds[0, 0]], dtype=np.float32)
        ML_prediction_start_time = time.time()

        # Use signatures approach instead of predict
        input_tensor = tf.convert_to_tensor(input_left)
        prediction_left = model.signatures['serving_default'](
            **{'conv1d_input': input_tensor})
    ...
    if side == 'right':
        # Convert to float32
        input_right = np.array([pred_ds[0, 1]], dtype=np.float32)
        ML_prediction_start_time = time.time()

        # Use signatures approach instead of predict
        input_tensor = tf.convert_to_tensor(input_right)
        prediction_right = model.signatures['serving_default'](
            **{'conv1d_input': input_tensor})
        # Extract from result dictionary if needed
...
def experimental_prediction(image: np.ndarray, side='both', cluster=True, display=False):
    """Takes an input experimental image, and outputs the predicted contact
    angle based on the contour input model found in this folder:
    './modules/ML_model/'
    """
    ...
    if side == 'left':
        ...
        both_inputs = prepare4model_v03_img(
            image, input_len=1223, cluster=cluster, display=display)
