...
def cluster_optics(sample, out_style: str = 'coords', xi: float = None, eps: float = None, verbose: int = 0):
...
def sim_bashforth_adams(h, a=1, b=1, num=500, all_the_way=False):
    ...
    '''
    def height(t, y, a, b): return y[1] - h
    ...
    if fit_type in FitType:
        ...
        *z, r = res['x']
        ...
        if fit_type == 'circular':
            ...
            θs, pred, Bo = sim_bashforth_adams(
                h, cap_length, curv, profile.shape[0])
            ϕ['left'] = -np.min(θs)
            ϕ['right'] = np.max(θs)
...
def yl_fit(profile, lim=10, fit_type=FitType.BASHFORTH_ADAMS, display=False):
    ...
    a = [
        CPs[0][1],
        (CPs[1][1]-CPs[0][1])/(CPs[1][0]-CPs[0][0])
    ]
...
if 0:
    ...
    angles, Bo, baseline_width, volume, fit, baseline, errors, sym_errors, timings = analyze_frame(
        img, 15, FitType.BASHFORTH_ADAMS, display=False)
