...
def ellipse_line_intersection(xc, yc, a, b, theta, x0, y0, x1, y1, display=False):
    ...
    B = -2*np.sin(theta)*np.cos(theta)/b**2 + 2 * \
        np.sin(theta)*np.cos(theta)/a**2
    C = np.cos(theta)**2/b**2 + np.sin(theta)**2/a**2
    D = - 2*xc*np.sin(theta)**2/b**2 + 2*yc*np.sin(theta)*np.cos(theta)/b**2 - \
        2*xc*np.cos(theta)**2/a**2 - 2*yc*np.sin(theta)*np.cos(theta)/a**2
    E = 2*xc*np.sin(theta)*np.cos(theta)/b**2 - 2*yc*np.cos(theta)**2/b**2 - \
        2*xc*np.sin(theta)*np.cos(theta)/a**2 - 2*yc*np.sin(theta)**2/a**2
    F = -1 + xc**2*np.sin(theta)**2/b**2 - 2*xc*yc*np.sin(theta)*np.cos(theta)/b**2 + yc**2*np.cos(theta)**2 / \
        b**2 + xc**2*np.cos(theta)**2/a**2 + 2*xc*yc*np.sin(theta) * \
        np.cos(theta)/a**2 + yc**2*np.sin(theta)**2/a**2
    ...
    # If the discriminant is positive, there are two intercepts
        ...
        grad_1 = -(2*((-xc + x_int_1)*np.sin(theta) + (-yc + y_int_1)*np.cos(theta))*np.sin(theta)/b**2 + 2*((-xc + x_int_1)*np.cos(theta) + (-yc + y_int_1)*np.sin(theta))*np.cos(theta)/a**2) / \
            (2*((-xc + x_int_1)*np.sin(theta) + (-yc + y_int_1)*np.cos(theta))*np.cos(theta)/b**2 +
             2*((-xc + x_int_1)*np.cos(theta) + (-yc + y_int_1)*np.sin(theta))*np.sin(theta)/a**2)
        grad_2 = -(2*((-xc + x_int_1)*np.sin(theta) + (-yc + y_int_2)*np.cos(theta))*np.sin(theta)/b**2 + 2*((-xc + x_int_2)*np.cos(theta) + (-yc + y_int_2)*np.sin(theta))*np.cos(theta)/a**2) / \
            (2*((-xc + x_int_2)*np.sin(theta) + (-yc + y_int_2)*np.cos(theta))*np.cos(theta)/b**2 +
             2*((-xc + x_int_2)*np.cos(theta) + (-yc + y_int_2)*np.sin(theta))*np.sin(theta)/a**2)
...
def ellipse_closest_point(xp, yp, xc, yc, a, b, th, n=1000, display=False):
    ...
    x = xc + a * np.cos(t) * np.cos(np.deg2rad(th)) - b * \
        np.sin(t) * np.sin(np.deg2rad(th))
    y = yc + a * np.cos(t) * np.sin(np.deg2rad(th)) + b * \
        np.sin(t) * np.cos(np.deg2rad(th))
...
def ellipse_fit_img(img, display=False):
    ...
    outputs = ellipse_line_intersection(t[0], t[1], a, b, math.radians(
        phi_deg), CPs[0][0], CPs[0][1], CPs[1][0], CPs[1][1])
