...
def test_perform_fits_circle(experimental_drop):
    ...
    print(
        f"circle fit - Circle center: {circle_center}, Radius: {circle_radius}")
...
def test_perform_fits_ellipse(mock_ellipse, experimental_drop):
    ...
    required_keys = ['left angle', 'right angle',
                     'ellipse center', 'ellipse a and b', 'ellipse rotation']
...
if __name__ == "__main__":
    ...
    test_data_path = os.path.join(os.path.dirname(
        __file__), 'test_data', 'test_drop_data.npz')
    if not os.path.exists(test_data_path):
        print("=" * 80)
