...
def experimental_drop():
    ...
    if not os.path.exists(test_data_path):
        pytest.fail(f"Test data file does not exist: {test_data_path}\nPlease run the main program to generate test data first")
    ...
    drop.contact_points = [np.array([89., 208.]), np.array([233., 207.])]
...
def test_perform_fits_tangent(experimental_drop):
    ...
    right_angle_original = experimental_drop.contact_angles['tangent fit']['right angle']
    ...
    print(f"tangent fit - Original left angle: {left_angle_original:.2f}°, Normalized: {left_angle:.2f}°")
    print(f"tangent fit - Original right angle: {right_angle_original:.2f}°, Normalized: {right_angle:.2f}°")
    print(f"tangent fit - Expected left angle: {EXPECTED_ANGLES['tangent fit']['left angle']}° ± {EXPECTED_ANGLES['tangent fit']['tolerance']}°")
    print(f"tangent fit - Expected right angle: {EXPECTED_ANGLES['tangent fit']['right angle']}° ± {EXPECTED_ANGLES['tangent fit']['tolerance']}°")
    
    # Check if angles are within expected range
    expected = EXPECTED_ANGLES['tangent fit']
    assert abs(left_angle - expected['left angle']) <= expected['tolerance'], \
        f"Left angle {left_angle:.2f}° is outside expected range {expected['left angle']}° ± {expected['tolerance']}°"
    assert abs(right_angle - expected['right angle']) <= expected['tolerance'], \
...
def test_perform_fits_polynomial(experimental_drop):
    ...
    # Extract calculated angles
    left_angle_original = experimental_drop.contact_angles['polynomial fit']['left angle']
    right_angle_original = experimental_drop.contact_angles['polynomial fit']['right angle']
    ...
    # Print calculated angles for debugging
    print(f"polynomial fit - Original left angle: {left_angle_original:.2f}°, Normalized: {left_angle:.2f}°")
    print(f"polynomial fit - Original right angle: {right_angle_original:.2f}°, Normalized: {right_angle:.2f}°")
    print(f"polynomial fit - Expected left angle: {EXPECTED_ANGLES['polynomial fit']['left angle']}° ± {EXPECTED_ANGLES['polynomial fit']['tolerance']}°")
    print(f"polynomial fit - Expected right angle: {EXPECTED_ANGLES['polynomial fit']['right angle']}° ± {EXPECTED_ANGLES['polynomial fit']['tolerance']}°")
...
def test_perform_fits_circle(experimental_drop):
    ...
    # Print calculated angles for debugging
    print(f"circle fit - Original left angle: {left_angle_original:.2f}°, Normalized: {left_angle:.2f}°")
    print(f"circle fit - Original right angle: {right_angle_original:.2f}°, Normalized: {right_angle:.2f}°")
    print(f"circle fit - Expected left angle: {EXPECTED_ANGLES['circle fit']['left angle']}° ± {EXPECTED_ANGLES['circle fit']['tolerance']}°")
    print(f"circle fit - Expected right angle: {EXPECTED_ANGLES['circle fit']['right angle']}° ± {EXPECTED_ANGLES['circle fit']['tolerance']}°")
...
def test_perform_fits_ellipse(mock_ellipse, experimental_drop):
    ...
    # Print calculated angles for debugging
    print(f"ellipse fit - Original left angle: {left_angle_original:.2f}°, Normalized: {left_angle:.2f}°")
    print(f"ellipse fit - Original right angle: {right_angle_original:.2f}°, Normalized: {right_angle:.2f}°")
    print(f"ellipse fit - Expected left angle: {EXPECTED_ANGLES['ellipse fit']['left angle']}° ± {EXPECTED_ANGLES['ellipse fit']['tolerance']}°")
    print(f"ellipse fit - Expected right angle: {EXPECTED_ANGLES['ellipse fit']['right angle']}° ± {EXPECTED_ANGLES['ellipse fit']['tolerance']}°")
    ...
    print(f"ellipse fit - Center: {ellipse_center}, a and b: {ellipse_ab}, Rotation: {ellipse_rotation}°")
...
def test_perform_fits_YL(experimental_drop):
    ...
    # Print calculated angles for debugging
    print(f"YL fit - Original left angle: {left_angle_original:.2f}°, Normalized: {left_angle:.2f}°")
    print(f"YL fit - Original right angle: {right_angle_original:.2f}°, Normalized: {right_angle:.2f}°")
    print(f"YL fit - Expected left angle: {EXPECTED_ANGLES['YL fit']['left angle']}° ± {EXPECTED_ANGLES['YL fit']['tolerance']}°")
    print(f"YL fit - Expected right angle: {EXPECTED_ANGLES['YL fit']['right angle']}° ± {EXPECTED_ANGLES['YL fit']['tolerance']}°")
    
    # Check if angles are within expected range, use larger tolerance for YL method
    expected = EXPECTED_ANGLES['YL fit']
    assert abs(left_angle - expected['left angle']) <= 5.0, \
        f"Left angle {left_angle:.2f}° is outside expected range {expected['left angle']}° ± 10.0°"
    assert abs(right_angle - expected['right angle']) <= 5.0, \
    ...
    if 'bond number' in experimental_drop.contact_angles['YL fit']:
        print(f"YL fit - Bond number: {experimental_drop.contact_angles['YL fit']['bond number']}")
    if 'volume' in experimental_drop.contact_angles['YL fit']:
        print(f"YL fit - Volume: {experimental_drop.contact_angles['YL fit']['volume']}")

# Run tests
if __name__ == "__main__":
    # Check if test data exists, if not print guidance
    test_data_path = os.path.join(os.path.dirname(__file__), 'test_data', 'test_drop_data.npz')
    if not os.path.exists(test_data_path):
        print("=" * 80)
